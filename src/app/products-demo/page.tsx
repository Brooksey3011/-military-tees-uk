"use client"

import { useState } from "react"
import { Layout } from "@/components/layout"
import { ProductGrid, ProductFilters, QuickViewModal } from "@/components/product"
import { CartSummary } from "@/components/cart"
import { Button } from "@/components/ui/button"
import { useCart } from "@/hooks/use-cart"
import type { Product, ProductVariant, FilterOptions, Category } from "@/types"

// Demo data
const demoCategories: Category[] = [
  { id: "1", name: "Armoury", slug: "armoury", created_at: "2024-01-01", updated_at: "2024-01-01", sort_order: 1, is_active: true },
  { id: "2", name: "Mess Hall", slug: "mess-hall", created_at: "2024-01-01", updated_at: "2024-01-01", sort_order: 2, is_active: true },
  { id: "3", name: "NAAFI", slug: "naafi", created_at: "2024-01-01", updated_at: "2024-01-01", sort_order: 3, is_active: true },
  { id: "4", name: "The Ranges", slug: "ranges", created_at: "2024-01-01", updated_at: "2024-01-01", sort_order: 4, is_active: true },
]

const demoProducts: Product[] = [
  {
    id: "1",
    name: "Regiment Pride T-Shirt",
    slug: "regiment-pride-tshirt",
    description: "Premium cotton t-shirt featuring classic regimental insignia and military heritage design.",
    price: 24.99,
    category_id: "1",
    main_image_url: "/placeholder-tshirt.jpg",
    is_featured: false,
    is_active: true,
    created_at: "2024-01-01",
    updated_at: "2024-01-01",
    category: demoCategories[0],
    variants: [
      {
        id: "1-1",
        product_id: "1",
        size: "M",
        color: "Olive",
        sku: "REG-PRIDE-M-OLIVE",
        stock_quantity: 15,
        price_adjustment: 0,
        is_active: true,
        image_urls: ["/placeholder-tshirt.jpg"],
        created_at: "2024-01-01",
        updated_at: "2024-01-01"
      },
      {
        id: "1-2", 
        product_id: "1",
        size: "L",
        color: "Olive",
        sku: "REG-PRIDE-L-OLIVE", 
        stock_quantity: 3,
        price_adjustment: 0,
        is_active: true,
        image_urls: ["/placeholder-tshirt.jpg"],
        created_at: "2024-01-01",
        updated_at: "2024-01-01"
      }
    ],
    reviews: [
      {
        id: "1",
        product_id: "1",
        customer_id: "1",
        rating: 5,
        comment: "Excellent quality!",
        created_at: "2024-01-01",
        updated_at: "2024-01-01"
      }
    ]
  },
  {
    id: "2",
    name: "Tactical Operations Tee",
    slug: "tactical-ops-tee",
    description: "Rugged design inspired by special operations units. Built for comfort and durability.",
    price: 29.99,
    category_id: "1",
    main_image_url: "/placeholder-tshirt.jpg",
    is_featured: false,
    is_active: true,
    created_at: "2024-01-01",
    updated_at: "2024-01-01",
    category: demoCategories[0],
    variants: [
      {
        id: "2-1",
        product_id: "2",
        size: "M",
        color: "Black",
        sku: "TAC-OPS-M-BLACK",
        stock_quantity: 0,
        price_adjustment: 0,
        is_active: true,
        image_urls: ["/placeholder-tshirt.jpg"],
        created_at: "2024-01-01",
        updated_at: "2024-01-01"
      }
    ]
  },
  {
    id: "3",
    name: "Mess Hall Classics",
    slug: "mess-hall-classics",
    description: "Celebrating military dining traditions with vintage-inspired designs.",
    price: 22.99,
    category_id: "2",
    main_image_url: "/placeholder-tshirt.jpg",
    is_featured: false,
    is_active: true,
    created_at: "2024-01-01",
    updated_at: "2024-01-01",
    category: demoCategories[1],
    variants: [
      {
        id: "3-1",
        product_id: "3",
        size: "S",
        color: "Khaki",
        sku: "MESS-CLASSIC-S-KHAKI",
        stock_quantity: 8,
        price_adjustment: 0,
        is_active: true,
        image_urls: ["/placeholder-tshirt.jpg"],
        created_at: "2024-01-01",
        updated_at: "2024-01-01"
      }
    ]
  },
  {
    id: "4",
    name: "NAAFI Store Heritage",
    slug: "naafi-heritage",
    description: "Honoring the Navy, Army and Air Force Institutes with authentic vintage styling.",
    price: 26.99,
    category_id: "3",
    main_image_url: "/placeholder-tshirt.jpg",
    is_featured: false,
    is_active: true,
    created_at: "2024-01-01",
    updated_at: "2024-01-01",
    category: demoCategories[2],
    variants: [
      {
        id: "4-1",
        product_id: "4",
        size: "L",
        color: "Navy",
        sku: "NAAFI-HERITAGE-L-NAVY",
        stock_quantity: 12,
        price_adjustment: 0,
        is_active: true,
        image_urls: ["/placeholder-tshirt.jpg"],
        created_at: "2024-01-01",
        updated_at: "2024-01-01"
      }
    ]
  }
]

export default function ProductsDemo() {
  const { addItem } = useCart()
  const [filters, setFilters] = useState<FilterOptions>({
    categories: [],
    colors: [],
    sizes: [],
    priceRange: [0, Infinity],
    sortBy: "newest"
  })
  const [favorites, setFavorites] = useState<string[]>([])
  const [quickViewProduct, setQuickViewProduct] = useState<Product | null>(null)

  const availableColors = ["Olive", "Black", "Khaki", "Navy"]
  const availableSizes = ["S", "M", "L", "XL"]

  const handleQuickView = (product: Product) => {
    setQuickViewProduct(product)
  }

  const handleAddToCart = (product: Product, variant: ProductVariant, quantity = 1) => {
    addItem(product, variant, quantity)
  }

  const handleToggleFavorite = (productId: string) => {
    setFavorites(prev => 
      prev.includes(productId)
        ? prev.filter(id => id !== productId)
        : [...prev, productId]
    )
  }

  return (
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          {/* Filters Sidebar */}
          <div className="w-full lg:w-64 flex-shrink-0">
            <ProductFilters
              filters={filters}
              onFiltersChange={setFilters}
              categories={demoCategories}
              availableColors={availableColors}
              availableSizes={availableSizes}
              priceRange={[0, 50]}
              compact={true}
            />
          </div>

          {/* Products Grid */}
          <div className="flex-1">
            <div className="mb-6">
              <h1 className="text-3xl font-display font-bold mb-2">
                Military Tees Collection
              </h1>
              <p className="text-muted-foreground">
                Discover our premium collection of British Army themed apparel
              </p>
            </div>

            <ProductGrid
              products={demoProducts}
              onQuickView={handleQuickView}
              onAddToCart={handleAddToCart}
              onToggleFavorite={handleToggleFavorite}
              favoriteProducts={favorites}
              filters={filters}
              columns={{
                mobile: 1,
                tablet: 2,
                desktop: 3
              }}
            />
          </div>
        </div>

        {/* Quick View Modal */}
        <QuickViewModal
          product={quickViewProduct}
          isOpen={!!quickViewProduct}
          onClose={() => setQuickViewProduct(null)}
          onAddToCart={(product, variant, quantity) => {
            handleAddToCart(product, variant, quantity)
            setQuickViewProduct(null)
          }}
          onToggleFavorite={handleToggleFavorite}
          isFavorite={quickViewProduct ? favorites.includes(quickViewProduct.id) : false}
        />
      </div>
    </Layout>
  )
}