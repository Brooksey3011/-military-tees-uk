  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        ...
        // Fetch products from marine-related categories
        const marineCategories = ['ops-room', 'training-wing', 'ranges', 'signals', 'mess-hall']
        ...
        // Remove duplicates and filter for marine-themed products
        const uniqueProducts = allProducts
          .filter((product, index, self) => 
            index === self.findIndex(p => p.id === product.id)
          )
          .filter(product => 
            product.name.toLowerCase().includes('marine') ||
            product.name.toLowerCase().includes('commando') ||
            product.name.toLowerCase().includes('amphibious') ||
            product.name.toLowerCase().includes('naval') ||
            product.description.toLowerCase().includes('marine') ||
            product.description.toLowerCase().includes('sea')
          )
        
        setProducts(uniqueProducts)
      } catch (err) {
        console.error('Error fetching Royal Marines products:', err)
    ...
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredAndSortedProducts.map((product) => (
            <ProductCard
              ...
              product={{
                ...product,
                category_id: product.category?.name || "royal-marines"
              }}
              variant="default"
            />
          ))}
      ...
      {!loading && filteredAndSortedProducts.length > 0 && (
        <div className="text-center text-sm text-muted-foreground">
          Showing {filteredAndSortedProducts.length} Royal Marines products
        </div>
      )}
    </div>
  )
