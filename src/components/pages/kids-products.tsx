...
export function KidsProducts({ onProductCountChange }: KidsProductsProps = {}) {
  ...
  const filteredAndSortedProducts = products
    .filter(product => {
      ...
      if (priceFilter === 'under15') return product.price < 15
      if (priceFilter === '15-20') return product.price >= 15 && product.price <= 20
      if (priceFilter === 'over20') return product.price > 20
      return true
    })
    .sort((a, b) => {
      switch (sortBy) {
  ...
  const handleWishlistToggle = (product: Product) => {
    if (isInWishlist(product.id)) {
      ...
      addToWishlist({
        ...
        category: product.category?.name || "kids",
        inStock: true,
        ...
        <div className={cn(
          ...
          {filteredAndSortedProducts.map((product) => {
            ...
            const normalizedProduct = {
              ...product,
              category_id: product.category?.slug || 'kids',
